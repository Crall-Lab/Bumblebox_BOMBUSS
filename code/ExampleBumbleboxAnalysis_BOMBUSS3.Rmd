...
## First loop and compile across individual tracking files to compile data
...
```
## Compile data on within-trial average data
...
## Individual spatial variation
```{r spatial variation of individuals}
#Plot for single representative individual, with brood locations overlaid
ggplot(subset(tracking_data, ID == 9), aes(x = centroidX, y = centroidY)) + geom_hex()+ggtitle('worker 9')+geom_point(data=brood_metadata, aes(x = x, y = y, colour = label))

#Plot spatial distribution for select individuals (representing centrality range)
p1 <- ggplot(subset(tracking_data, ID == 1), aes(x = centroidX, y = centroidY)) + geom_hex(show.legend = FALSE)+ggtitle('worker 1')+geom_point(data=brood_metadata, aes(x = x, y = y, colour = label, alpha = 0.7), show.legend = FALSE)
p2 <- ggplot(subset(tracking_data, ID == 13), aes(x = centroidX, y = centroidY)) + geom_hex(show.legend = FALSE)+ggtitle('worker 13')+geom_point(data=brood_metadata, aes(x = x, y = y, colour = label, alpha = 0.7), show.legend = FALSE)
p3 <- ggplot(subset(tracking_data, ID == 15), aes(x = centroidX, y = centroidY)) + geom_hex(show.legend = FALSE)+ggtitle('worker 15')+geom_point(data=brood_metadata, aes(x = x, y = y, colour = label, alpha = 0.7), show.legend = FALSE)
p4 <- ggplot(subset(tracking_data, ID == 3), aes(x = centroidX, y = centroidY)) + geom_hex(show.legend = FALSE)+ggtitle('worker 3')+geom_point(data=brood_metadata, aes(x = x, y = y, colour = label, alpha = 0.7), show.legend = FALSE)
...
#Load contact data by looping over source files
